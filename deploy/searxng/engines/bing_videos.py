...
ZenAtlas patch of SearXNG 2026.9.16+f725cc793 (mounted over the image's copy by compose.discovery.yml):
Bing mixes result cards without the ``vrhdata`` metadata block into its video pages. Upstream raises on
the first such card and discards the whole page; this copy skips that card instead. Drop the mount once
upstream handles it.
"""
...
def response(resp):
    ...
    for result in dom.xpath('//div[contains(@id, "mc_vtvc_video")]'):
        raw = eval_xpath_getindex(result, './/div[@class="vrhdata"]/@vrhm', index=0, default=None)
        try:
            metadata = json.loads(raw) if raw else None
        except ValueError:
            metadata = None
        if not isinstance(metadata, dict) or not metadata.get("murl"):
            continue
