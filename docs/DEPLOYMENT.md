...
Use `npm run start` for the API and `npm run worker` for a **separate supervised process**, both from the repository root. `Dockerfile` builds the Node service and can run either command. Browser page checks (`PAGE_RENDERS`) run inside the worker and need Chromium on that host: `npx playwright install --only-shell chromium`, or build the image with `--build-arg PAGE_RENDER=true`. Chromium keeps its sandbox on, so a container must allow user namespaces (Docker's default seccomp profile does on most hosts); if Chromium cannot start, the worker logs `browser_launch_failed`, retries after five minutes and keeps plain page checks running. Do not disable the sandbox, because rendered pages are untrusted. For main-content text (`PAGE_TEXT_PYTHON`), the worker host also needs Python with `pip install -e "scene-worker[pages]"`; the helper only parses HTML and needs no network access. The optional scene worker is a third supervised process: Python 3.11+, installed with `pip install -e scene-worker` (add the `transcribe` extra for faster-whisper), started with `python -m zenatlas_scenes work` from the repository root. It needs the runtime `DATABASE_URL`, `GEMINI_API_KEY`, and read access to `SCENE_MEDIA_ROOT`; `Dockerfile` does not include it. Keep the Gemini key server-side and send media only from sources whose reviewed policy permits `video_analysis`. A static host cannot execute this engine. Short-lived/serverless web routes need continuously running worker/SearXNG compute elsewhere. No deployment credentials or existing hosting were supplied.
...
The SearXNG overlay has no invented default version. Resolve an actual official image digest, record its matching source revision and license notices, then set `SEARXNG_IMAGE`. Enable JSON output and confirm your selected engine works. A JSON 403 can mean JSON format is disabled; CAPTCHA, throttling or upstream blocking must be reported/handled, not bypassed. The provided limiter-off SearXNG settings require its loopback/private-only placement; do not expose that instance directly to the public. The overlay also mounts a patched Bing Videos engine (`deploy/searxng/engines/bing_videos.py`, written for image 2026.9.16+f725cc793) and sets `PYTHONPYCACHEPREFIX` so the image's precompiled bytecode does not shadow it. When you select a different image, check that the patch still matches its engine, or remove the mount. The settings also enable the deep-dive engines (`SEARXNG_DEEP_ENGINES`, `SEARXNG_DEEP_WEB_ENGINES`); a deep dive sends each engine many requests, one at a time, over about two minutes, so watch the `searxng:<engine>` counters in `/api/admin/health` for engines that start refusing. Every search now uses Gemini at least twice and a deep dive at least four times: Gemini's free tier (20 requests a day per Flash model) is not enough for regular use.
...
Supervise `worker-main.ts` with your container orchestrator/service manager, and run one `watchdog-main.ts` (`npm run watchdog`) beside it. The API, worker and watchdog each write a heartbeat every 15 seconds. The watchdog reports a stopped or stuck worker, searches waiting in the queue, and failing dependencies (SearXNG engines, Gemini models, YouTube, AniList, Chromium, packages and known vulnerabilities) at `GET /api/admin/dependencies` and on the admin page, and can post status changes to a webhook; see [WATCHDOG.md](WATCHDOG.md). In a separate container, set `WATCHDOG_API_URL` to the API's service address. The loop polls persisted schedules; no browser timer drives indexing. On graceful stop the current bounded job finishes. A crash leaves a 90-second lease; another worker reclaims it. Job transitions use lease tokens to prevent stale completion, and metadata ingestion is idempotent. Three failed attempts terminate a job. A stalled or failed source backs off and can be paused automatically. Inspect `GET /api/admin/health` and structured events `worker_cycle_failed`, `request_failed`, and `database_pool_error` without logging raw requests/credentials.
...
3. Use your SearXNG instance directly from server compute: `/search?q=Blender&format=json&engines=youtube` (repeat for each configured engine; adding `categories` would enable the whole category). Confirm JSON and upstream status, then check the API's sparse/refresh search, that polling shows results while discovery runs, and that **Dig deeper** adds a "Deep dive finds" section within about three minutes.
