...
- `GET /api/admin/sources`: newest 200 sources including candidates and policies; bearer token required.
...
- `PATCH /api/admin/sources/:id`: body follows `examples/source-policy.json`; validates a review note, status, retention period, metadata/transcript permissions, adapter and optional feed URL.
- `GET /api/admin/health`: provider failure counters, queue status counts and oldest queued timestamp. It exposes operational information only to administrators.
