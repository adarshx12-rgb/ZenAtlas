...
**Jev source exploration:** the same OpenRouter key also enables up to four page visits across two rounds, following real references toward new sources. New candidates still pass normal relevance checks. `JEV_EXPLORATION_ENABLED=false` disables this independently of screening. See [exploration settings and trace export](docs/DISCOVERY_QUALITY.md#jev-source-exploration) to evaluate its discoveries.
...
Start the worker, then use **Fresh discovery**. Catalogue results return immediately. Discovery collects all launched provider responses, checks a bounded shared candidate pool, and publishes its final relevance ranking. Failures and incomplete coverage are reported; existing catalogue results survive. Repeated normalized queries reuse their active or recent completed job for 10 minutes by default. See [discovery quality](docs/DISCOVERY_QUALITY.md) for budgets and limits.
...
- **Quick** (default): searches the typed query and AI-planned variants on standard engines. Final discovery results appear after the sources and relevance checks finish.
- **Deep dive** (2–3 minutes): the results page shows **Dig deeper** under the result count once a quick search has finished (not in catalogue-only mode). It calls `POST /api/search/:id/deep` and goes down the rabbit hole for underrated sources that ordinary searches miss:
  ...
  - after the search window, all launched requests settle; quick and deep candidates are checked together, then sorted by descending relevance. The final list can place a specialist result above an earlier quick result. Obscurity is a badge, never a score boost.

  A deep dive only runs when someone asks for it.

The steps:

- **Anime recognition** (`src/anilist.ts`, `ANILIST_ENABLED`, no key needed): before planning, checks whether the query confidently names a specific anime, using the public [AniList](https://anilist.co) GraphQL API in up to three stages, stopping at the first that succeeds:
  1. **By title**, directly: most of a matched title's own words appear in the query (a short, canonical title against a longer request), or most of the query's words appear in the title (a verbose or compound title, such as an arc-specific entry, that adds words no query would include).
  2. **By title, narrowed**: AniList's own search is title-literal, so "attack on titan season 4 episode 28" finds nothing directly; season/episode markers and trailing descriptive words are stripped for a second, narrower attempt.
  3. **By character**, for a query that names people or an event rather than the show (for example "gohan ssj2 vs cell", which shares no words with "Dragon Ball Z"): every remaining significant word is searched in parallel as a character name. Character search is noisy on ordinary English words (several plainly non-anime words each coincide with a real, sometimes popular character in testing), so a show only counts when at least two independently searched words each name a character best known for it, and each of those characters clears a modest popularity floor; a single hit, or two hits on different shows, decides nothing.

  A confident match's official English, romaji and native titles, synonyms, genres, format, episode count, studios, season and, when the request names a specific character, scene or moment, up to 15 streaming-service episode titles that relate to it (`matching_episode_titles`; best effort — coverage varies a great deal by show, and thematic titles rarely spell out a scene's exact keywords, so this hints at the AI's own knowledge of the show rather than deciding the episode by itself) are given to the planner (to search under the show's real names and, for a specific scene, name its likely episode, season or arc) and the judge (to recognise fan-subbed, dubbed or renamed uploads of it, and to prefer candidates naming the identified episode or arc). It is catalogue data, not instructions, and never the user's own text. Every stage shares `ANILIST_DAILY_BUDGET` (a query needing all three stages costs up to seven requests); a miss adds no notice, since most searches are not about anime.
- **Planning** (`src/planner.ts`, needs `OPENROUTER_API_KEY` with `PLANNER_MODELS`, or `GEMINI_API_KEY`): one request classifies the search as videos, websites or both, writes up to `PLAN_SEARCHES` queries from different angles (expert terms, synonyms, `site:` queries for platforms and showcases where such work is published), and lists up to five checkable criteria. The user's own query always runs too. The models in `PLANNER_MODELS` do the planning — the first leads and supplies the kind and criteria, the rest assist and have their queries merged into the same capped list. Gemini plans only when no models are named, or as a last resort when every one of them has failed, so an ordinary search leaves its quota for judging and scene analysis. Without any of them, or if planning fails outright, the query runs as typed, on web engines when it mentions websites and on video engines otherwise.
...
`public/results.js` renders the complete visible `ranked` snapshot as one ordered list, updating card positions when discovery completes. It cancels or ignores superseded requests and displays incomplete-coverage notices. API searches expire after 30 minutes. Paginated consumers should refresh their first page at completion because discovery can change positions; catalogue-only pagination stays stable.
...
| `JUDGE_MODELS` | Empty; comma-separated OpenRouter models that rank results, tried in order. Must accept image input. Gemini judges only when this is empty, or as a last resort when every named model fails |
| `JUDGE_MODEL` | Empty (uses `GEMINI_MODEL`); the Gemini model used when Gemini does the planning or judging |
...
| `OPENROUTER_BASE_URL`, `OPENROUTER_API_KEY` | `https://openrouter.ai/api/v1`; empty key switches the OpenAI-compatible assist planners off |
| `OPENROUTER_SITE_URL`, `OPENROUTER_SITE_NAME` | Optional attribution headers OpenRouter shows for the app; sent only when set |
| `PLANNER_MODELS`, `PLANNER_ASSIST_TIMEOUT_MS` | Empty; comma-separated OpenRouter models that plan searches, first leading and the rest assisting, their queries merged into one capped list. Gemini plans only when this is empty, or as a last resort when every named model fails. An assist not answering within 6000 ms is left out; the first model is awaited in full |
...
[Learning loop](docs/LEARNING.md) records every discovery search, lets searchers rate results and report what was missing, and has a critic model (Sonnet 5 by default) audit each search, testing claimed missing sources with real searches. Off unless `CRITIC_ENABLED=true`.

[Video evidence and real-query evaluation](docs/VIDEO_EVIDENCE.md) covers PeerTube/Archive adapters, broader comment sampling, caption retention, automatic scene scheduling, semantic passage retrieval and human relevance review.
