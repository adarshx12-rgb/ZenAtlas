...
Each step is optional and independent: without keys, or when a step fails or runs out of budget, discovery keeps its keyword ranking and the search response reports that step as `partial`, `unavailable` or `budget_exhausted`. Results carry `badges` (for example `Livestream replay`, `Official channel`, `Discussed on Reddit`, `Underrated find`) and `judgement` (`relevance`, `reason`, `model`). Choose **Viewer timestamps** in the evidence filter to search stored viewer moments in catalogue mode. Review the [YouTube API Services Developer Policies](https://developers.google.com/youtube/terms/developer-policies) and your Gemini terms before production use.
