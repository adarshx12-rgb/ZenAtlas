...
Discovery has two depths (`depth` in the search API, `src/discovery.ts`):

- **Quick** (default): the query runs as typed, on web engines when it mentions websites and on video engines otherwise, and results are keyword-ranked and shown as each engine answers (typically 3–6 seconds). No AI, page or comment checks run.
- **Deep dive**: the results page shows **Dig deeper** under the result count once a quick search has finished (not in catalogue-only mode). It calls `POST /api/search/:id/deep`, which keeps everything already shown and runs every step below (typically 30–60 seconds). New finds still appear as they arrive; when the checks finish, the AI ranking reorders the discovered results, adds its reasons, and removes the ones it rejected, including results the quick search had shown. A deep dive only runs when someone asks for it.

A deep dive runs these steps:
...
- **Searching**: every planned query runs in parallel on every provider. For SearXNG, video queries use `SEARXNG_ENGINES` and web queries use `SEARXNG_WEB_ENGINES` (Google, Bing, Brave and Yahoo answered on 16 September 2026; DuckDuckGo showed a CAPTCHA, Qwant denied access and Mojeek returned nothing). SearXNG requests are capped by `SEARXNG_DAILY_BUDGET`; paid Google/Brave requests by `DISCOVERY_DAILY_BUDGET`. Each lead is scored against the words of the query that found it (operators such as `site:` are ignored), leads found by several queries rank higher, and up to `DISCOVERY_RESULTS` new results are kept. The checks and the AI judgement below also cover the quick search's results for the same query.
...
- **AI judgement** (`GEMINI_API_KEY`, `JUDGE_MODEL`, `src/judge.ts`): structured requests score the top `JUDGE_CANDIDATES` results 0–10 against the request and the plan's criteria, from their title, channel, description, comments, viewer moments, Reddit mentions and page check, and give a short reason. The model may only pick moments it was given; unknown results and invented moment IDs are discarded. Results scored 2 or lower are dropped. Uploads that look like unofficial full copies of films or TV episodes are scored down, and channels listed in `OFFICIAL_YOUTUBE_CHANNELS` are labelled and preferred for scene requests. Calls are capped by `JUDGE_DAILY_BUDGET`; candidate text, including comments, is sent to Google.

Each step is optional and independent: without keys, or when a step fails or runs out of budget, discovery keeps its keyword ranking and the search response reports that step as `partial`, `unavailable` or `budget_exhausted`. Results carry `badges` (for example `Livestream replay`, `Official channel`, `Discussed on Reddit`) and `judgement` (`relevance`, `reason`, `model`). Choose **Viewer timestamps** in the evidence filter to search stored viewer moments in catalogue mode. Review the [YouTube API Services Developer Policies](https://developers.google.com/youtube/terms/developer-policies) and your Gemini terms before production use.

Each deep dive uses at least two Gemini requests (planning and judging), more when a model falls back or `JUDGE_BATCH_SIZE` splits the judgement; quick searches use none. Keys with low per-minute limits return rate-limit errors under repeated searches; the search then continues without that step and says so.
