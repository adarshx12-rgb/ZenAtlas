...
- **Planning** (`src/planner.ts`, needs `GEMINI_API_KEY` or, failing that, `OPENROUTER_API_KEY` with `PLANNER_ASSIST_MODELS`): one request classifies the search as videos, websites or both, writes up to `PLAN_SEARCHES` queries from different angles (expert terms, synonyms, `site:` queries for platforms and showcases where such work is published), and lists up to five checkable criteria. The user's own query always runs too. Gemini leads when it has a key, and each `PLANNER_ASSIST_MODELS` model plans alongside it on OpenRouter, merging their queries; with no Gemini key the first assist becomes the primary planner instead. Without either, or if planning fails, the query runs as typed, on web engines when it mentions websites and on video engines otherwise.
...
| `PLANNER_ASSIST_MODELS`, `PLANNER_ASSIST_TIMEOUT_MS` | Empty; comma-separated OpenRouter models that plan alongside Gemini and merge their queries with its own (the first becomes the primary planner when `GEMINI_API_KEY` is empty); an assist not answering within 6000 ms is left out |
