...
Discovery trades a few seconds for accuracy (typically 10–30 seconds per fresh search, `src/discovery.ts`):
...
Each fresh search uses at least two Gemini requests (planning and judging), more when a model falls back or `JUDGE_BATCH_SIZE` splits the judgement. Keys with low per-minute limits return rate-limit errors under repeated searches; the search then continues without that step and says so.
