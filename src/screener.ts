...
export interface ScreenResult { promising: Set<string>; screened: number; decisions?: ScreenDecision[] }
export interface Screener { screen(query: string, candidates: DiscoveryCandidate[]): Promise<ScreenResult> }
...
export class JevScreener implements Screener {
 ...
 async screen(query: string, candidates: DiscoveryCandidate[]): Promise<ScreenResult> {
   ...
   // At most six concurrent calls, each with one deadline and no retries. A failed
   // batch restores the entire original ordering rather than favoring successful batches.
   const settled = await Promise.allSettled(batches.map(async batch => {
     const state = {request: query, candidates: Object.fromEntries(batch.map((c, i) => [`c${i}`, {
     ...
     const parsed = response.safeParse(await this.transport(url.href, {
   ...
   return {screened: selected.length, promising: new Set(decisions.filter(d => d.promoted).map(d => d.url)), decisions};
 }
}
