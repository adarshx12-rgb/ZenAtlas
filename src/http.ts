...
type Options = { timeoutMs?: number; maxBytes?: number; method?: 'GET'|'POST'|'HEAD'; body?: unknown;
 ...
 headers?: Record<string,string>; probe?: boolean };
...
export async function fetchJSON(input: string, options: Options = {}): Promise<any> {
 ...
 for (let redirects = 0; redirects <= (options.redirects ?? 2); redirects++) {
   ...
   const response = await new Promise<{status: number; location?: string; data: string}>((resolve, reject) => {
     let req: http.ClientRequest | undefined;
     const timer = setTimeout(() => { req?.destroy(); reject(new UpstreamError('timeout')); }, remaining);
     const finish = (error?: Error, value?: {status: number; location?: string; data: string}) => {
     ...
     void (async () => {
       ...
       req = (url.protocol === 'https:' ? https : http).request(url, {
         ...
         headers: { Accept: options.probe?'*/*':'application/json', 'Accept-Encoding': 'identity', 'User-Agent': 'ZenAtlas/0.1',
         ...
         if (status >= 300 && status < 400 && res.headers.location) {
           res.destroy(); finish(undefined, {status, location: res.headers.location, data: ''}); return;
         }
         if(options.probe){res.destroy();finish(undefined,{status,data:''});return;}
         ...
         if (!(options.contentTypes ?? ['application/json','application/feed+json']).includes(type) ||
         ...
         res.on('end', () => finish(undefined, {status, data: Buffer.concat(chunks).toString('utf8')}));
   ...
   if(options.probe)return {status:response.status,url:url.href,redirects:chain} satisfies ProbeResponse;
