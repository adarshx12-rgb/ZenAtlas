...
import { enqueue, claim, complete, renewLease } from '../src/queue.js';
...
test('leases recover after worker death, fence stale completion, and expire retained data',async()=>{
 ...
 try{
   ...
   await db.query("UPDATE jobs SET lease_until=now()+interval '5 seconds' WHERE id=$1",[recovered.id]);
   await renewLease(db,old);
   assert.equal((await db.query("SELECT lease_until>now()+interval '60 seconds' AS renewed FROM jobs WHERE id=$1",[recovered.id])).rows[0].renewed,false,'a stale worker cannot renew the lease');
   await renewLease(db,recovered);
   assert.equal((await db.query("SELECT lease_until>now()+interval '60 seconds' AS renewed FROM jobs WHERE id=$1",[recovered.id])).rows[0].renewed,true,'the current worker renews while waiting for providers');
